...
public class GrupoManagedBean implements Serializable {
	...
	public List<SelectItem> getSelectOneItemGrupo() {
		...
		for (Grupo grupo : grupos) {
			SelectItem selectItem = new SelectItem(grupo.getId(),
					grupo.getNombre());
			...
			try {
				...
				if (getGrupoService().addGrupo(grupoAdd)) {
					...
					FacesMessage message = new FacesMessage(
							FacesMessage.SEVERITY_INFO, "Exito : ", "El grupo "
									+ grupoAdd.getNombre()
									+ " se guardo con éxito :)");
	...
	public void onEdit(ActionEvent actionEvent) {
		try {
			...
			if (getGrupoService().updateGrupo(getSelectedGrupo())) {
				...
				FacesMessage message = new FacesMessage(
						FacesMessage.SEVERITY_INFO, "Exito : ", "El grupo "
								+ this.selectedGrupo.getNombre()
								+ " se modifico con éxito :)");
				FacesContext.getCurrentInstance().addMessage(null, message);
			} else {
				FacesMessage message = new FacesMessage(
						FacesMessage.SEVERITY_ERROR, "Error : ", "El grupo "
								+ this.selectedGrupo.getNombre()
