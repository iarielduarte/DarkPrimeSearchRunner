...
public class RolDao implements IRolDao, Serializable {
	...
	public Rol getRolById(int id) {
		Query query = getSessionFactory().getCurrentSession()
				.getNamedQuery(Rol.GET_ROL_BY_ID).setInteger("id", id);
	...
	// public Rol getRolById(int id) {
	// List list =
	// getSessionFactory().getCurrentSession().createQuery("FROM Rol WHERE id=?").setParameter(0,
	// id).list();
	...
	// public List<Rol> getRols() {
	// List list =
	// getSessionFactory().getCurrentSession().createQuery("FROM Rol").list();
	// return list;
	// }
